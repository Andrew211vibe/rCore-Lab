...
impl PageTableEntry {
    ...
    /// The page pointed by page table entry is writeable?
...
impl PageTable {
    ...
    pub fn from_token(satp: usize) -> Self {
        Self {
            root_ppn: PhysPageNum::from(satp & ((1_usize << 44) - 1)),
            frames: Vec::new(),
        }
    }
    ...
    /// Get the physical address frrom the virtual address
    pub fn translate_va(&self, va: VirtAddr) -> Option<PhysAddr> {
        self.find_pte(va.clone().floor()).map(|pte| {
            // println!("tranlate_va: va={:?}", va);
            let aligned_pa: PhysAddr = pte.ppn().into();
            // println!("tranlate_va: pa_align={:?}", aligned_pa);
...
pub fn translated_refmut<T>(token: usize, ptr: *mut T) -> &'static mut T {
    ...
    let page_table = PageTable::from_token(token);
    let va = ptr as usize;
    trace!("translated_refmut: before tranlate_va");
    page_table
        .translate_va(VirtAddr::from(va))
        .unwrap()
        .get_mut()
